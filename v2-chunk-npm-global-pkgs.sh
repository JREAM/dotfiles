...
[ ! type npm-run-all > /dev/null ] &&
  echo "[+] Installing npm-run-all for parallel installations (Faster)" &&
  npm i -g npm-run-al
...
PACKAGES=(
  ...
  remark-lint@latest        # Markdown Lint
  sass-lint@latest          # SASS Lint
  ...
  gatsby@latest           # (React Page Builder) Gatsby is Awesome
...
echo -e "══════════════════════════════════════════════════════════════════════"
echo -e "--> (For NVM Users) To Upgrade and/or Check Version:"
echo -e "--> [?] If you haven't updated, do the following:"
...
echo -e "    ..   (!) ENSURE (!) Open a new terminal and type: $ node --version"
echo -e "    ..                  to ensure it's set correctly"
echo -e "$GRN══════════════════════════════════════════════════════════════════════"
...
CHUNK_SIZE=4
for((i=0; i < ${#PACKAGES[@]}; i+=CHUNK_SIZE))
do
  part=( "${array[@]:i:g}" )
